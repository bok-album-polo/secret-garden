...
def copy_public_site_example_pages(config: Dict[str, Any]) -> None:
    """
    Recursively copies example pages and subdirectories from 
    service/public-site-example-pages/ into src/Views/ for each public site.
    """
    global BUILD_DIR, REPO_ROOT
    repo_root = REPO_ROOT if REPO_ROOT is not None else (BUILD_DIR.parent if BUILD_DIR is not None else Path(__file__).resolve().parents[2])
    build_dir = BUILD_DIR if BUILD_DIR is not None else (repo_root / 'build')
    ...
    print("\nCopying example pages (recursively) to public sites:")
    ...
    for idx, site in enumerate(config.get("public_sites", []), start=1):
        ...
        try:
            # dirs_exist_ok=True allows us to merge this content into the existing Views folder
            # generated by the initial clone.
...
def main():
    ...
    copy_public_site_example_pages(config)
