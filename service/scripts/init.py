...
def generate_public_config_php(config: Dict[str, Any]) -> None:
    ...
    for idx, site in enumerate(config.get("public_sites", []), start=1):
        ...
        payload['database_server'] = {
            ...
            'db_name': site_db_server.get('db_name') or db_server.get('db_name', 'secret_garden'),
...
def generate_admin_config_php(config: Dict[str, Any]) -> None:
    ...
    payload['application_config'] = {
        'password_generated_length': app_cfg.get('password_generated_length'),
        'password_generated_charset': app_cfg.get('password_generated_charset')
    }
...
def copy_public_site_example_pages(config: Dict[str, Any]) -> None:
    ...
    print("\nCopying example pages (recursive) to public sites:")
